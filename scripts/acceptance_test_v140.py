#!/usr/bin/env python3
"""
数据集管理模块 全量验收测试 v1.4.0
====================================
对照验收清单 V1.4.0 + 测试用例 V1.4.0，通过 HTTP 直接访问 Docker 8091。

运行方式：
  python3 scripts/acceptance_test_v140.py [--base-url http://127.0.0.1:8091]

前置条件：
  - Docker 容器运行于 8091（`docker compose up -d`）
  - 宿主机 MySQL 可连（127.0.0.1:3306 / eye_research_dataset）
  - 宿主机 FTP 运行于 0.0.0.0:2121（python3 scripts/run_local_ftp_server.py --host 0.0.0.0）

测试开始前自动清空全部 MySQL 数据 + FTP 文件，测试结束后再次清理测试数据。
"""
...
ALL_TABLES = TABLES_WITH_DIR + ["dataset_merged_file", "dataset_upload_task", "dataset_upload_part", "export_record"]
...
def expect(item_id: str, desc: str, fn, *args, **kwargs):
    """运行 fn(*args, **kwargs)，捕获断言 / 异常，记录 PASS/FAIL。"""
    try:
        fn(*args, **kwargs)
...
# 测试数据构造
# ─────────────────────────────────────────────────────────────────────────────
STUB_JPEG = bytes([0xFF, 0xD8, 0xFF, 0xE0]) + b"\x00\x10JFIF\x00\x01\x01\x00\x00\x01\x00\x01\x00\x00" + b"\xFF\xD9"
...
def build_test_zip(pid1="ACC_PT_001", pid2="ACC_PT_002") -> bytes:
    """问卷 + 眼底 JPEG FDT + OCT dat/json，用于导入验收。"""
    xbio = io.BytesIO()
    wb = Workbook()
    ws = wb.active
...
def run(base_url: str) -> None:
    ...
    def _db_parsed_path():
        rows = mysql_rows("dataset_image_asset", "directory_id=%s AND parsed_path IS NOT NULL", (directory_id,))
    ...
    def _ftp_oct_frames():
        ...
        if not rows:
            # 没有标记为 OCT 解析的资产；可能只有 PARSED_FDT，打印已有类型供排查
            all_types = mysql_rows("dataset_image_asset", "directory_id=%s", (directory_id,), cols="source_type")
            print(f"  [debug] source_types in DB: {[r['source_type'] for r in all_types]}")
            return  # 无 OCT DAT 解析产物时豁免
        ...
        if parsed_path.endswith(".png"):
            ...
            assert png_frames, f"FTP 目录 {parent} 无 frame_*.png（nlst 结果: {frames[:5]}）"
    ...
    def _timeline():
        tl = ok(client.get(f"/api/v1/dataset-directories/{directory_id}/patients/{pid1}/timeline"))
    ...
    tl_data = ok(client.get(f"/api/v1/dataset-directories/{directory_id}/patients/{pid1}/timeline"))
    survey_date = tl_data["dates"][0]["surveyDate"]

    def _images_list():
        imgs = ok(client.get(f"/api/v1/dataset-directories/{directory_id}/patients/{pid1}/images",
    ...
    imgs_data = ok(client.get(f"/api/v1/dataset-directories/{directory_id}/patients/{pid1}/images",
        params={"surveyDate": survey_date, "pageNo": 1, "pageSize": 20}))
    ...
    def _images_frameurl():
        ...
        non_oct = [r for r in imgs_data["records"] if r["sourceType"] not in ("PARSED_DAT", "PARSED_OCT_DAT")]
        for r in non_oct:
    ...
    def _images_missing_date():
        r = client.get(f"/api/v1/dataset-directories/{directory_id}/patients/{pid1}/images")
    ...
    expect("I-37", "API-15/GET dataset-files thumbnail 200 + image/*", _thumbnail)
    ...
    # OCT frame — 优先用 API-15 的 frameUrl；如果没有则直接从 DB 取 parsed_path 构造
    oct_imgs = [r for r in imgs_data["records"] if r.get("frameUrl")]
    if not oct_imgs:
        # 从 DB 取已有帧路径直接构造
    ...
    # ── Phase 5: 目录导出 + API-18/19 + 下载 ────────────────────────────────────
    section("Phase 5 │ I-31～I-33/I-46～I-48/X-01/X-06 目录导出全链路")
    ...
    def _dir_ftp_path():
        rows = mysql_rows("export_record", "export_record_id=%s", (dir_exp_id,), cols="ftp_path,export_status")
    ...
    def _export_list_basic():
        ...
        assert rec["expireAt"]
        assert rec["summary"]
    expect("I-46/I-46a/X-09", "API-18 导出列表含 DONE 目录导出，字段完整", _export_list_basic)
    ...
    expect("I-47/X-10", "API-19 目录导出详情字段完整，payload 含 directoryIds", _export_detail)
    ...
    if zip_content:
        def _dir_zip_parsed_images():
            ...
            names = zf.namelist()
            parsed = [n for n in names if "_parsed_derived" in n or ".jpg" in n.lower() or ".png" in n.lower()]
            assert parsed, f"zip 内无解析图像，条目={names[:10]}"
        expect("I-33-parsed", "目录导出 zip 包含解析图像（jpg/png）", _dir_zip_parsed_images)

    # ── Phase 6: 患者导出 + API-18/19 + 下载验证 ────────────────────────────────
    section("Phase 6 │ I-41～I-43/I-46～I-48 患者导出全链路")
    ...
    expect("I-46a", "API-18 列表同时含目录导出和患者导出两类", _export_list_both)
    ...
    expect("I-47/X-10", "API-19 患者导出详情字段完整，payload 含 patientId", _pat_exp_detail)

    # 患者导出下载 & 内容验证
    pat_dl_url = pat_exp_done["downloadUrl"]

    def _pat_download_zip():
        r = client.get(pat_dl_url)
        assert r.status_code == 200
        zf = zipfile.ZipFile(io.BytesIO(r.content))
        names = zf.namelist()
    ...
    rows_json = None
    try:
        rows_json = _pat_download_zip()
        check("I-43/X-06", "患者导出 zip 含 questionnaire_rows.json + images/ + images/parsed/", True)
    except Exception as e:
        check("I-43/X-06", "患者导出 zip 含 questionnaire_rows.json + images/ + images/parsed/", False, str(e)[:200])
    ...
    expect("I-43", "患者导出 surveyDates=['2026-05-20'] 仅含指定日期行", _pat_survey_dates_filter)

    # ── Phase 7: 删除目录 ────────────────────────────────────────────────────────
    section("Phase 7 │ I-29/I-30 删除目录")
    ...
    expect("I-30", "删除后目录不再出现在列表中", _deleted_not_in_list)
    ...
    expect("I-30", "删除后 records 接口返回 404", _deleted_records_gone)

    # ── Phase 8: 事后清理 ────────────────────────────────────────────────────────
    section("Phase 8 │ 清理测试数据")
